...
class CheckMatch extends \Bga\GameFramework\States\GameState
{
    ...
    public function onEnteringState(int $activePlayerId)
    {
        ...
        if (count($flippedCards) != 2) {
            ...
            static::DbQuery("UPDATE `card` SET `card_state` = 0 WHERE `card_state` = 1");
            return FirstSelection::class;
        }

        if ($cardValues[0]['color'] == $cardValues[1]['color']) {
            // MATCH!
            static::DbQuery("UPDATE `card` SET `card_state` = 2 WHERE `card_id` IN (" . implode(',', $cardIds) . ")");
            ...
            $remaining = self::DbGetOne("SELECT count(*) FROM `card` WHERE `card_state` = 0");
            if ($remaining == 0) {
            ...
            static::DbQuery("UPDATE `card` SET `card_state` = 0 WHERE `card_id` IN (" . implode(',', $cardIds) . ")");
